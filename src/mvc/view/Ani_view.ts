 ...
 */
class Ani_view extends Base_view{
    private chat_texts = [
        "大家好，很高兴见到各位",
        "快点吧，我等的花都谢了",
        "跟你合作真是太愉快了",
    ...
    //开混动画
    public kai_hun_ani(hunPai_model:Base_card_model,back_fun){
    ...
    //开局-补花动画
    public start_flower_ani(ani_info){
        for(var i in ani_info.ani_list){
            var info = ani_info.ani_list[i];
            var point:egret.Point;
            switch (Number(info.position)){
                case 1: point = new egret.Point(Main.stageWidth/2,Main.stageHeight/2+150);break;
                case 2: point = new egret.Point(Main.stageWidth-200,Main.stageHeight/2);break;
                case 3: point = new egret.Point(Main.stageWidth/2,Main.stageHeight/2-150);break;
                case 4: point = new egret.Point(200,Main.stageHeight/2);break;
            }
            if(info.is_ani){
                var ani_detail = {
    ...
    //玩家出牌动画
    public game_user_send_card(info){
    ...
    //玩家打出去的牌 放大提示动画
    private max_tips_card:egret.Sprite;
    private max_tips_timer;
    public game_max_card_tips(info){
        this.clear_max_tips_card();
        if(info){
            this.max_tips_card=new egret.Sprite();
            this.addChild(this.max_tips_card);
            this.max_tips_timer=setTimeout(function () {
                if(!this.max_tips_card)return;
                var bg=this.set_bit_center("g_max_bg");
                this.max_tips_card.addChild(bg);
                bg.y=49;
                bg.x=-1;
        ...
        if(info.type != this.CONST.PLAYER_ACTION.guo){
            ...
            switch (info.type){
                ...
                case this.CONST.PLAYER_ACTION.hu:
                    ...
                    if(info.position==1)action_sound_url+="_"+huType+"_as";//自摸-自己听到-胡的就是这张
                    else action_sound_url+="_"+huType;//自摸-别人听到-不好意思自摸了
                    break;//胡
                case this.CONST.PLAYER_ACTION.guo:
                    if(info.position!=1)action_sound_url="";
            ...
            if(action_sound_url)Sound_model.playSoundEffect(action_sound_url);//自己听到-过的声音
        }
    }
    //流局动画
    public liu_ju_ani(info){
        if(info.back_fun){
            //播放动画
    ...
    //文字
    public addTextAni(str):egret.TextField{
