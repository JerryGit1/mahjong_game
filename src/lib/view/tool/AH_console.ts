...
class MyConsole {
    ...
    public start(v,scene=null){
        ...
        if (this._isTrace) {
            if(window["ah_tester"]){
                ...
                //3.控象系统
                if(this.currentVersionType!="release"&&scene)this.start_control_system(scene);
            }
        }
    }
    /**
    ...
    private start_control_system(scene:egret.Sprite){
        var control_sp:egret.Sprite;//物体范围可视化
        var control_view:egret.DisplayObjectContainer;
        var speed=1;//移动速度
        var control_view_w;
        var control_view_h;
        var control_view_txt:egret.TextField;
            ...
            switch (e.keyCode){
                case 49://1 控物  键盘1
                    if(current_control_type==0){
                        ...
                        if(this.ah_tester.start_cs)this.ah_tester.start_cs();
                    }else{
                        remove_sp();
                        if(this.ah_tester.close_cs)this.ah_tester.close_cs();
                        current_control_type=0;
                    }
                    break;
                case 50://2 控物--键盘控制  键盘2
                    if(current_control_type==0)return;
                    current_control_type=1;
                    break;
                case 188://speed
                    speed+=.5;
                ...
                case 190://speed
                    ...
                    break;
                case 51://2 控物--鼠标控制  键盘3
                    if(current_control_type==0)return;
                    current_control_type=2;
                    break;
                case 52://2 测物--鼠标控制  键盘4
                    if(current_control_type==0)return;
                ...
                case 39://右
                    if(control_view&&current_control_type==1)control_view.x+=speed;
                    break;
                case 37://左
                    if(control_view&&current_control_type==1)control_view.x-=speed;
                    break;
                case 38://上
                    if(control_view&&current_control_type==1)control_view.y-=speed;
                    break;
                case 40://下
                    if(control_view&&current_control_type==1)control_view.y+=speed;
                    break;
        ...
        var canvas_y=0;
        var canvas_x=0;
        function control_system_choose_view(e:egret.TouchEvent){
            if(current_control_type==3)return;
