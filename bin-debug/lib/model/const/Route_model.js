 ...
 */
var Route_model = (function () {
    function Route_model() {
        this.service_to_local_list = [];
        this.local_to_service_list = [];
        this.fields_analysis_list = [];
        /*--------------------------------通过-接口-进行路由字段解析----------------------------------*/
        // //服务器内容key值对应
    ...
    /*tyq: 解析接受到的服务器内容*/
    Route_model.prototype.get_server_to_local_by_route = function (data) {
        for (var key in data) {
    ...
    /*tyq: 解析本地发起的数据*/
    Route_model.prototype.get_local_to_server_by_route = function (data) {
    ...
    //服务器内容key值对应
    Route_model.prototype.get_server_new_key = function (old_key) {
        var n_k = "";
        ...
        if (n_k == "") {
            if (!Number(old_key) && Number(old_key) != 0)
                MyConsole.getInstance().trace("重大失误-----服务器所传字段不在解析表中：" + old_key, 0);
            n_k = old_key;
        }
        return n_k;
    ...
    //本地内容key值对应
    Route_model.prototype.get_local_new_key = function (old_key) {
        var n_k = "";
    ...
    /*--------------------------------通过-文件-进行路由字段解析----------------------------------*/
    Route_model.prototype.set_fields_analysis_by_file = function () {
        MyConsole.getInstance().trace("四号机:对通信字段编码 进行解析", 2);
...
__reflect(Route_model.prototype, "Route_model");
//# sourceMappingURL=Route_model.js.map