 ...
 */
var Popup_model = (function (_super) {
    __extends(Popup_model, _super);
    ...
    Popup_model.prototype.set_model = function (socket_model, ani_model) {
        this.socket_model = socket_model;
        this.ani_model = ani_model;
        this.port_socket();
    };
    //监听服务器给的消息;
    Popup_model.prototype.port_socket = function () {
        //大厅战绩查询;
    ...
    //网络断开连接;
    Popup_model.prototype.socket_close = function () {
        this.m_to_c_dis_event(this.EVENT.socket.socket_close);
    };
    Popup_model.prototype.socket_unusual = function (str) {
        this.m_to_c_dis_event(this.EVENT.socket.socket_unusual, str);
    };
    //通用浮层提示框
    Popup_model.prototype.popup_float_alert = function (args) {
        var data = {};
        data["str"] = args[0];
    ...
    //zpb:俱乐部 我的信息弹窗
    Popup_model.prototype.club_my_info = function (data) {
        this.m_to_c_dis_event(this.EVENT.base.club_my_info_popup, data);
    };
    //zpb:俱乐部 申请离开
    Popup_model.prototype.club_request_leave = function () {
        this.m_to_c_dis_event(this.EVENT.base.club_request_leave_popup);
    };
    Popup_model.prototype.get_club_get_my_info = function (data) {
        data.page = this.current_page;
        this.m_to_c_dis_event(this.EVENT.base_popup.club_add_my_info, data);
    };
    //zpb:关闭所有弹窗
    Popup_model.prototype.close_all_popup = function () {
    };
    //服务器返回的----大厅战绩查询-------数据给 control;
    Popup_model.prototype.hall_achievement = function (info) {
        this.m_to_c_dis_event(this.EVENT.popup.hall_achievement, info);
    };
    //创建房间-回应;
    Popup_model.prototype.hall_createRoom = function (info) {
        //1：创建房主房间;
        if (info.reqState == 1) {
            //扣除房卡
            var create_money;
            if (Number(info.roomInfo.circleNum) == 2) {
                create_money = 4;
            }
            else if (Number(info.roomInfo.circleNum) == 4) {
                create_money = 6;
            }
            else if (Number(info.roomInfo.circleNum) == 8) {
                create_money = 12;
            }
    ...
    //加入房间;
    Popup_model.prototype.hall_joinRoom = function (info) {
    ...
    Popup_model.prototype.hall_currentReplaceRoom = function (info) {
        this.issue_list_info = info;
        this.m_to_c_dis_event(this.EVENT.popup.issue_info, info);
    };
    //代开房间用户加入;
    Popup_model.prototype.player_join_room = function (info) {
    ...
    //代开模式房主解散房间;
    Popup_model.prototype.hall_dissolveReplaceRoom = function (info) {
        if (info.reqState == 1) {
            //返回设置自己的房卡数;
    ...
    //代开房间解散房间;
    Popup_model.prototype.hall_issueRoom_delete = function (info) {
        if (info.extraType == 1) {
            this.send_issue_info(this.CONST.USERID);
        }
    };
    //代开房间解散房间回应;
    Popup_model.prototype.hall_orderDissolveReplaceRoom = function (info) {
        if (info.reqState == 1) {
            this.send_issue_info(this.CONST.USERID);
        }
    };
    //代开历史;
    Popup_model.prototype.hall_historyReplaceRoom = function (info) {
        this.m_to_c_dis_event(this.EVENT.popup.issue_history, info);
    };
    //创建房间;
    Popup_model.prototype.send_create_room = function (data) {
        if (data.clubId) {
    ...
    //加入房间;
    Popup_model.prototype.join_create_room = function (data) {
        this.socket_model.join_room(data);
    };
    //向服务器请求战绩列表;
    Popup_model.prototype.send_record_info = function (userId, pages) {
        /****************用于生成假数据 别删********************/
        //假数据;
        // var data={};
        // data["userId"]=userId;
    ...
    //向服务器请求代开列表;
    Popup_model.prototype.send_issue_info = function (userId) {
        this.socket_model.get_issue_replace(userId);
    };
    //删除代开房间用户;
    Popup_model.prototype.send_delete_user = function (data) {
        this.socket_model.delete_user(data);
    };
    //解散房间;
    Popup_model.prototype.send_dissolution_room = function (roomId) {
        var data = {};
        data["roomId"] = roomId;
        this.socket_model.delete_room(data);
    };
    //强制解散房间;
    Popup_model.prototype.send_qz_dissolution_room = function (roomId) {
        var data = {};
        data["roomId"] = roomId;
        this.socket_model.order_delete_room(data);
    };
    //请求代开历史;
    Popup_model.prototype.send_issue_history = function (data) {
        /****************用于生成假数据 别删********************/
        // var info={};
    ...
    //俱乐部创建房间
    Popup_model.prototype.club_createRoom = function (info) {
        this.hall_createRoom(info);
    };
    //俱乐部-加入房间请求---发起socket
    Popup_model.prototype.club_join_room_send = function (roomId, userId) {
        this.socket_model.club_join_room({ userId: userId, roomId: roomId });
    };
    //俱乐部-加入房间---回应socket
    Popup_model.prototype.club_join_room = function (info) {
        this.hall_joinRoom(info);
    };
    //代开房间，有用户加入---变化本地数据，不去和服务器交互了;
    Popup_model.prototype.issue_user_join_room = function (info) {
        for (var i = 0; i < this.issue_list_info.length; i++) {
    ...
    //代开房间，有用户离开;
    Popup_model.prototype.issue_user_leave_room = function (info) {
        /*
    ...
    Popup_model.prototype.record_bogus_info = function (info) {
    };
    Popup_model.prototype.club_history_info = function (info) {
    };
    return Popup_model;
}(Scene_model));
__reflect(Popup_model.prototype, "Popup_model");
//# sourceMappingURL=Popup_model.js.map